  ...
  return cache.NewSharedIndexInformer(
    &cache.ListWatch{
      ListFunc: func(opts metav1.ListOptions) (runtime.Object, error) {
        ...
        err := client.Get().Namespace(namespace).Resource("channels").VersionedParams(&opts, scheme.ParameterCodec).Timeout(timeout).Do(ctx).Into(result)
        return result, err
      },
      WatchFunc: func(opts metav1.ListOptions) (watch.Interface, error) {
        ...
        opts.Watch = true
        return client.Get().Namespace(namespace).Resource("channels").VersionedParams(&opts, scheme.ParameterCodec).Timeout(timeout).Watch(ctx)
      },
    },
    obj,
    30*time.Second,
